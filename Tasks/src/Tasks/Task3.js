...
function setStat(stat, player, points) {
	console.log(points);
	let _stat = prompt(`${player} Input ${stat} `);
	if (Number(_stat) > points) {
		alert(`${stat} must be under or equal ${points}`);
		setStat(stat, player, points);
	}
	console.log({ _stat });
	return _stat;
}
...
function fighter() {
	...
	function doAction(name, me, target, fighter) {
		const input = prompt(`${fighter}: ${name} wants to...`);
		if (input.toLowerCase() === 'takedamage') {
			takeDamage(me);
		} else if (input.toLowerCase() === 'dealdamage') {
			dealDamage(target);
		} else if (input.toLocaleLowerCase() === 'getname') {
			getName(me);
		} else if (input.toLocaleLowerCase() === 'gethp') {
			getHp(me);
		} else {
			alert('type takeDamage or dealDamage ONLY');
			doAction(name, me, target, fighter);
		}
	}

	let p1Name = prompt('Player1 Input your Name');
	let p1Strength = setStat('Stretgth', 'Player1', 30);
	console.log({ p1Strength });
	let p1Agility = setStat('Agility', 'Player1', 30 - p1Strength);
	console.log(p1Agility);
	let p1Vitality = setStat('Vitality', 'Player1', 30 - p1Agility - p1Strength);
	let player1 = new Fighter(p1Name, p1Strength, p1Agility, p1Vitality);
	console.log(player1);

	let p2Name = prompt('Player2 Input your Name');
	let p2Strength = setStat('Stretgth', 'Player2', 30);
	console.log({ p2Strength });
	let p2Agility = setStat('Agility', 'Player2', 30 - p2Strength);
	console.log(p2Agility);
	let p2Vitality = setStat('Vitality', 'Player2', 30 - p2Agility - p2Strength);
	...
	while (player1.health > 0 && player2.health > 0) {
		if (player1.health < 0 || player2.health < 0) {
			...
			doAction(p1Name, player1, player2, 'Fighter1');
		}

		if (player1.health < 0 || player2.health < 0) {
			break;
		} else {
			doAction(p2Name, player2, player1, 'Fighter2');
		}
	}
...
export default function Fighter1() {
	return (
		<div className="container m-5 text-center">
			<h2>Task 3</h2>
			<h2>Fighter</h2>
			<button onClick={() => fighter()}>Start Fight</button>
		</div>
	);
}
